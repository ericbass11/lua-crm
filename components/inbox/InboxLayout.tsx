...
// `Button` já vem importado no topo (era o par do "Nova conversa" desta
// instalação); a sincronização trouxe a mesma linha do upstream.
...
export function InboxLayout({ initialSelectedId = null }: InboxLayoutProps = {}) {
  ...
  /** "Nova conversa" (envio ativo) — extensão desta instalação. */
  ...
  return (
    <div className="grid h-[calc(100dvh-3.5rem-2*var(--space-6))] w-full grid-cols-1 md:grid-cols-[300px_1fr] xl:grid-cols-[272px_1fr_296px] 2xl:grid-cols-[300px_1fr_320px]">
      ...
      */}
      <div
        className={cn(
          // `bg-surface` é do design system Indigo desta instalação; o resto
          // (colunas responsivas) é do upstream.
          "h-full min-h-0 flex-col bg-surface md:flex",
          colunas.conversa,
        )}
      >
        {/*
