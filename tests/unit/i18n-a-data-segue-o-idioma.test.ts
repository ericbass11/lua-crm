...
const FORA_DE_INTERFACE: Record<string, string> = {
  // ─── Extensões desta instalação (fork LUA CRM) ───
  "lib/mystery/report.ts":
    "laudo do Cliente Oculto em PDF, entregue a uma clínica brasileira por WhatsApp — documento, não interface; o idioma é do produto (pt-BR), não do leitor logado",
  "lib/google/calendar.ts":
    "formata a descrição do evento gravado na agenda do Google — sai do produto; não há idioma de UI resolvido nesse caminho",
  "lib/ai/runtime/agent.ts":
    "carimbo de data/hora injetado no prompt do agente — segue a língua da persona, não a do operador que abriu a tela",
  "app/app/mystery/_client.tsx":
    "módulo Cliente Oculto é pt-BR por decisão de produto (laudo para clínicas brasileiras); i18n do módulo é pendência declarada da fork",
