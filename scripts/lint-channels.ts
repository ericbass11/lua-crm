...
const KNOWN_DEBT: { reason: string; files: string[] }[] = [
  ...
  {
    reason:
      "Extensões locais anteriores à adoção do seam: o enriquecimento de mídia " +
      "precisa receber a sessão para usar `fetchInboundMedia`; o Cliente Oculto " +
      "ainda precisa de capacidades de verificar destinatário e enviar arquivo " +
      "que o ChannelAdapter não expõe. A sincronização congela esses três pontos; " +
      "migrá-los exige ampliar o contrato e testar o fluxo real do módulo.",
    files: [
      "lib/ai/runtime/media.ts",
      "lib/mystery/engine.ts",
...
const offenders = ROOTS.flatMap(walk)
  .map((fsPath) => ({ fsPath, repoPath: fsPath.replaceAll("\\", "/") }))
  .filter(({ repoPath }) => !ALLOWED.some((re) => re.test(repoPath)))
  .filter(({ fsPath }) => nomeiaProvider(readFileSync(fsPath, "utf8")))
