...
export class WahaClient {
   ...
   *
   * Convive com `checkContactExists` (upstream) de propósito: os CONTRATOS são
   * diferentes e cada um tem chamador próprio. Este devolve `null` em falha,
   * porque quem o chama (Cliente Oculto, envio ativo) precisa DESISTIR em
   * silêncio — mandar para um chatId errado responde 201 e não entrega. O do
   * upstream LANÇA, porque o vcard precisa distinguir "não existe" de "a
   * checagem falhou". Unificá-los obrigaria um dos dois lados a tratar o erro
   * do jeito errado.
   */
  async checkExists(
    session: string,
  ...
  async sendFile(input: {
    ...
    const res = await this.fetchComTeto(`${this.baseUrl}/api/sendFile`, {
      method: "POST",
      headers: { "X-Api-Key": this.apiKey, "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  /**
   * Envia mídia (imagem/áudio/documento) conforme o plano resolvido em
   * `lib/messaging/media` — o endpoint e o payload já vêm decididos.
   */
  async sendMedia(
    session: string,
    chatId: string,
