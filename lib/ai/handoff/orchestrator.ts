...
export async function triggerHandoff(
  ...
  try {
    ...
    // Step 7 — notificação EXTERNA ao time (WhatsApp/webhook — extensão desta
    // instalação, `lib/notify/handoff.ts`). Best-effort; nunca bloqueia. Vem
    // DEPOIS do aviso na Central: o item interno é o alerta canônico, o externo
    // é a cópia que sai da casa. Sem ela, um handoff fica parado no inbox sem
    // ninguém de fora da tela saber.
    try {
