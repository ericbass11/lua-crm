...
|---|---|---|
| App embedded vs External | **External** | Mais flexibilidade de UI custom (admin LUA CRM tem UX própria); evita iframe sandbox; consent renderizado no domínio Nuvemshop é suficiente |
...
export class NuvemshopAdapter implements EcommercePlatformAdapter {
  ...
  private async request<T>(ctx: AdapterContext, path: string, init: RequestInit & { idempotencyKey?: string } = {}): Promise<{ data: T; rateLimit: FetchPage<T>['rateLimit'] }> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'lua-crm/1.0 (eric@lua-crm.example)',
...
- UI custom no admin LUA CRM (sem iframe Nuvemshop sandbox)
- Permite callback em domínio próprio com cookie de sessão LUA CRM
...
## 8. Mapping Nuvemshop → LUA CRM

### 8.1 `customer.email/phone/identification` → `contacts`

| Campo Nuvemshop | Campo LUA CRM | Notas |
|---|---|---|
